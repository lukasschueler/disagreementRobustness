...
class Rollout(object):
    def __init__(self, ob_space, ac_space, nenvs, nsteps_per_seg, nsegs_per_env, nlumps, envs, policy,
                 int_rew_coeff, ext_rew_coeff, record_rollouts, dynamics_list):
        self.nenvs = nenvs
    ...
    def calculate_reward(self):
        ...
        if self.dynamics_list[0].var_output:
            ...
            print("----------------SHAPE OF INTZRI9NSIC REWARDS--------------------")
            print(np.shape(var_rew))
            sys.exit("pass")

        self.loggingData(self, var_rew)
    ...
    def rollout_step(self):
        ...
        for l in range(self.nlumps):
            ...
            for info in infos:
                ...
                unityepinfo = info.get("unity_episode", {})
                epinfo.update(unityepinfo)
    ...
    def update_info(self):
        ...
        if all_ep_infos:
            ...
            if "unity_rooms" in keys_:
                ...
                self.all_visited_rooms.extend(temp)
                self.all_visited_rooms = sorted(list(set(self.all_visited_rooms)))
                self.stats["n_rooms"] = len(self.all_visited_rooms)
                if MPI.COMM_WORLD.Get_rank() == 0:
                    print("All visited levels")
                    print(self.all_visited_rooms)
